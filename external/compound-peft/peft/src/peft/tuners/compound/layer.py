...
class CompoundLayer(nn.Module, LycorisLayer):
    ...
    adapter_layer_names = ("compound", "compound_s")
    ...
    def __init__(self, base_layer: nn.Module):
        ...
        self.compound_s = nn.ParameterDict({})
        self.use_scaling = {}
        self.use_offset_blocks = {}

    @property
    def _available_adapters(self) -> Set[str]:
            return {*self.compound, *self.compound_s}
    ...
    def create_adapter_parameters(
        ...
        self.shape = shape
        self.compound_n, _ = self.find_closest_binomial(
            target=int(shape[0] / r), compound_pattern=compound_pattern
        )
        if block_share:
            self.compound[adapter_name] = nn.Parameter(
                torch.empty(num_adapters, 1, self.compound_n, self.compound_n)
            )
        else:
            self.compound[adapter_name] = nn.Parameter(
                torch.empty(num_adapters, r, self.compound_n, self.compound_n)
            )

        if use_scaling:
            self.compound_s[adapter_name] = nn.Parameter(torch.ones(shape[0], 1))

    def reset_adapter_parameters(self, adapter_name: str):
        if adapter_name in self.compound:
            nn.init.zeros_(self.compound[adapter_name])
        if adapter_name in self.compound_s:
            nn.init.ones_(self.compound_s[adapter_name])

    def reset_adapter_parameters_random(self, adapter_name: str):
        if adapter_name in self.compound:
            nn.init.xavier_uniform_(self.compound[adapter_name], gain=nn.init.calculate_gain("linear"))
        if adapter_name in self.compound_s:
            nn.init.normal_(self.compound_s[adapter_name], mean=1.0, std=0.1)
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        self.use_scaling[adapter_name] = use_scaling
        self.use_offset_blocks[adapter_name] = use_offset_blocks
        self.r = r
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[List[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self._available_adapters:
                ...
                if active_adapter in self.compound_s:
                    s = self.compound_s[active_adapter].view(-1)
                    new_weights = new_weights * s.unsqueeze(0)
                if isinstance(base_layer, nn.Linear):
    ...
    def get_delta_weight(self, adapter_name: str) -> torch.Tensor:
        ...
        use_offset = self.use_offset_blocks.get(adapter_name, False)
        weight = None
        ...
        for i in range(num_adapters):
            ...
            weight_i = self._block_diagonal(orth_rotate, compound_pattern, compound_type, block_share)

            # BOFT-inspired: shift block boundaries for adapters i>0 so successive
            # adapters cover different subspaces, making their product a full orthogonal
            # matrix rather than staying block-diagonal in the original basis.
            if use_offset and i > 0:
                d = weight_i.shape[0]
                offset = (i * d) // num_adapters
                perm = (torch.arange(d, device=weight_i.device) + offset) % d
                weight_i = weight_i[perm][:, perm]
    ...
    def _cayley_batch(self, data: torch.Tensor) -> torch.Tensor:
        ...
        # Solve (I+S)^T X^T = (I-S)^T  =>  X = (I-S)(I+S)^{-1}
        Q = torch.linalg.solve((I + skew).transpose(1, 2), (I - skew).transpose(1, 2)).transpose(1, 2)
    ...
    def _compute_circuit_compound_(self, unary, order: int = 1, compound_type: str = "comp"):
        ...
        if (order < 0) or (order > num_qubits):
            ...
            subsets_tensor = torch.tensor(subsets, dtype=torch.long, device=unary.device)
            submatrices = unary[subsets_tensor][:, :, subsets_tensor].transpose(1, 2).transpose(2, 3)
            if compound_type == "comp":
                submatrices_float = submatrices.float()  # Convert to float32
                compound = torch.linalg.det(submatrices_float)
                compound = compound.to(unary.dtype)
                ...
                raise ValueError(
                    f"Unsupported compound_type: {compound_type} | Please choose one of comp/max/avg/perm"
                )
...
class Linear(CompoundLayer):
    ...
    def _get_delta_activations(
        ...
        # Convert delta_weight to same dtype as input
        ...
        if input.shape[-1] != delta_weight.shape[0]:  # Compare c with r
            ...
            r = delta_weight.shape[0]
            pad_size = c - r
            padded_delta_weight = torch.zeros(c, c, device=delta_weight.device, dtype=delta_weight.dtype)
            padded_delta_weight[:r, :r] = delta_weight
            padded_delta_weight[r:, r:] = torch.eye(pad_size, device=delta_weight.device, dtype=delta_weight.dtype)
            delta_weight = padded_delta_weight

        result = torch.matmul(input, delta_weight)
        if adapter_name in self.compound_s:
            result = result * self.compound_s[adapter_name].view(1, -1)
        return result
